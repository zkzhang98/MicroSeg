...
class ExtRandomHorizontalFlip(object):
    ...
    def __call__(self, img, lbl, sal):
        ...
        if random.random() < self.p:
            return F.hflip(img), F.hflip(lbl), [F.hflip(x) for x in sal]
        return img, lbl, sal
...
class ExtCenterCrop(object):
    ...
    def __call__(self, img, lbl, sal):
        ...
        return F.center_crop(img, self.size), \
                ...
                [F.center_crop(x, self.size) for x in sal]
...
class ExtRandomScale(object):
    ...
    def __call__(self, img, lbl, sal):
        ...
        return F.resize(img, target_size, self.interpolation), \
                ...
                [F.resize(x, target_size, Image.NEAREST) for x in sal]
    ...
    def __call__(self, pic, lbl, sal):
        ...
        if self.normalize:
            return F.to_tensor(pic), \
                    ...
                    [torch.from_numpy( np.array( x, dtype=self.target_type) ) for x in sal]
        else:
...
class ExtRandomCrop(object):
    ...
    def __call__(self, img, lbl, sal):
        ...
        if self.padding > 0:
            ...
            sal = [F.pad(x, self.padding) for x in sal]

        # pad the width if needed
        if self.pad_if_needed and img.size[0] < self.size[1]:
            img = F.pad(img, padding=int((1 + self.size[1] - img.size[0]) / 2), fill=255)
            lbl = F.pad(lbl, padding=int((1 + self.size[1] - lbl.size[0]) / 2), fill=255)
            sal = [F.pad(x, padding=int((1 + self.size[1] - x.size[0]) / 2), fill=255) for x in sal]

        # pad the height if needed
        if self.pad_if_needed and img.size[1] < self.size[0]:
            img = F.pad(img, padding=int((1 + self.size[0] - img.size[1]) / 2), fill=255)
            lbl = F.pad(lbl, padding=int((1 + self.size[0] - lbl.size[1]) / 2), fill=255)
            sal = [F.pad(x, padding=int((1 + self.size[0] - x.size[1]) / 2), fill=255) for x in sal]

        i, j, h, w = self.get_params(img, self.size)

        return F.crop(img, i, j, h, w), F.crop(lbl, i, j, h, w), [F.crop(x, i, j, h, w) for x in sal]
...
class ExtResize(object):
    ...
    def __call__(self, img, lbl, sal):
        ...
        return F.resize(img, self.size, self.interpolation), \
                ...
                [F.resize(x, self.size, Image.NEAREST) for x in sal]
